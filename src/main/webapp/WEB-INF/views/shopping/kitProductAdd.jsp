...
<body>
			...
			<div class="row">
				...
				<div class="col-md-10">
					<div class="contact-form mb60">
						<h3>작물 판매 등록</h3>
						<form action="plantSaleInsert.do" onsubmit="return check();"
							method="post">
							<ul class="cform">
								<li class="full"><select class=" full form-control"
									id="plant_no" name="plant_no" onchange="selectFnc(this)">
										<option value="-1">작물목록</option>
										<c:forEach var="list" items="${selectMemList}">
											<option value="${list.plant_no }">작물 종류 :
												${list.plant_name } | 등급 : ${list.plant_grd } | 재고 :
												${list.plant_ivy } | 총 재배량 : ${list.plant_tot_grow_amnt }</option>
										</c:forEach>
								</select></li>
								...
								<li class="half pl-15"><input type="number"
									...
									placeholder="수량" min="0" oninput="countCheck()"
								...
								<li>
									<div class="filebox" style="float: right">
										<label for="ex_file">사진 업로드</label> <input type="file"
											id="ex_file">
									</div>
								</li>
	...
	<script type="text/javascript">
		...
		function countCheck() {
			...
			var index = $("#plant_no option").index(
					$("#plant_no option:selected")) - 1;
